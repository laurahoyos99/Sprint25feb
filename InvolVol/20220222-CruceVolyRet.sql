...
SELECT DISTINCT RIGHT(CONCAT('0000000000',ACT_ACCT_CD),10) AS ACT_ACCT_CD, EXTRACT(MONTH FROM MaxChurnVol) AS MES
FROM VOLUNTARYMAXCHURNERS
...
SELECT v.MES, COUNT(DISTINCT V.ACT_ACCT_CD) AS CrucevOL
FROM CONTRATOSVOLUNTARIOS v INNER JOIN CONTRATOSNORET r ON v.ACT_ACCT_CD=r.Contrato
