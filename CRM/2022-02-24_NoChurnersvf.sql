...
SELECT DISTINCT MO.MONTH
    ...
    , AM.MONTH_ACT
    , LAST_VALUE(CRM.VO_FI_TOT_MRC_AMT) OVER (PARTITION BY CRM.CST_CUST_ID, DATE_TRUNC(CRM.FECHA_EXTRACCION, MONTH)  ORDER BY CRM.FECHA_EXTRACCION) LAST_MRC_MONTH
    , LAST_VALUE(CRM.VO_FI_TOT_MRC_AMT_DESC) OVER (PARTITION BY CRM.CST_CUST_ID, DATE_TRUNC(CRM.FECHA_EXTRACCION, MONTH) ORDER BY CRM.FECHA_EXTRACCION) LAST_MRC_DESC_MONTH
FROM `gcp-bia-tmps-vtr-dev-01.gcp_temp_cr_dev_01.2022-02-16_FINAL_HISTORIC_CRM_FILE_2021_D` CRM
    ...
    INNER JOIN ACTIVE_MONTHS AM
        ON AM.ACT_ACCT_CD=CRM.ACT_ACCT_CD
    LEFT JOIN CHURN_MONTH_FILTER FM
        ON CRM.ACT_ACCT_CD = FM.ACT_ACCT_CD
WHERE CRM.PD_VO_PROD_NM IS NOT NULL
    AND FM.ACT_ACCT_CD IS NULL
)
, LAST_MRC AS (
SELECT MONTH
    , MONTH_ORDER_ROW
...
, PIVOT_VALS AS (
SELECT PD_VO_PROD_NM AS PRODUCT_NAME
    , CST_CUST_ID
    , MONTH_ACT AS CHURN_DATE
